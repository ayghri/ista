...
if __name__ == "__main__":
    # --- Configuration ---
    TARGET_SPARSITY = 0.1
    MODEL_NAME = f"ResNet50_CIFAR10_IHT_{TARGET_SPARSITY}"  # Choose ResNet18, ResNet34, ResNet50 etc.
    ...
    def set_sparsity(model, ratio):
        ...
        for name, p in model.named_parameters():
            ...
            if not (
                ...
                p.iht_k = int(p.numel() * ratio)
                # p.dystil_beta = DYSTIL_BETA
                print(name, p.numel(), p.shape, p.iht_k)
                p_used += p.iht_k
            else:
                p_used += p.numel()
