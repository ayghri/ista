...
def train(
    epoch, model, trainloader, optimizer, criterion, device, scheduler=None
):
    model.train()
    train_loss = 0
    correct = 0
    total = 0
    pbar = tqdm(trainloader, desc=f"Epoch {epoch+1} Training", leave=False)
...
if __name__ == "__main__":
    ...
    TARGET_SPARSITY = 0.2
    DYSTIL_BETA = 0.01
    MODEL_NAME = f"ResNet50_ImageNet_{TARGET_SPARSITY}"  # Choose ResNet18, ResNet34, ResNet50 etc.
    DATA_DIR = "/buckets/datasets/imagenet/official/archives"
    CHECKPOINT_DIR = "./checkpoint"
    EPOCHS = 200  # Needs enough epochs
    # Standard starting LR for SGD
    INITIAL_LR = 0.1 * (1 / TARGET_SPARSITY) ** 0.5
    BATCH_SIZE = 128
    MOMENTUM = 0.95
